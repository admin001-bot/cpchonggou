				...
				try{
					...
					var times=3000;
						if(ctype=='g1'){
							...
							if($dom.find('.feipan')) $dom.find('.feipan').html("快樂飛盤：<em>"+$feipan+"</em>");
						}else if(ctype=='g2'){ //快3
							$('.kj-hao li').each(function(i){
								times-=500;
					...
					$('#kaijiang #kjsay').hide();
					$('#kaijiang #kjsay').html('開獎倒數計時：<em class="kjtips">00:00</em>');
					$('.wjkjData p').hide();
					$('.wjkjData ul').show();
...
function safeBeforSetPwd(){
	if(!this.oldpassword.value){winjinAlert("請輸入原密碼","alert");return false;}
	if(this.oldpassword.value.length<6){winjinAlert("原密碼至少6位","alert");return false;}
	if(!this.newpassword.value){winjinAlert("請輸入新密碼","alert");return false;}
	if(this.newpassword.value.length<6){winjinAlert("密碼至少6位","alert");return false;}
	var confirmpwd=$(':password.confirm', this).val();
	if(confirmpwd!=this.newpassword.value){winjinAlert("兩次輸入密碼不一樣","alert");return false;}
	return true;
}
...
function safeBeforSetCBA(){
	if(!this.account.value){winjinAlert("銀行帳號沒有填寫","alert");return false;}
	if(!this.username.value){winjinAlert("銀行開戶名沒有填寫","alert");return false;}
	if(!this.coinPassword.value){winjinAlert("請輸入提款密碼","alert");return false;}
	if(this.coinPassword.value<6){winjinAlert("提領密碼至少6位","alert");return false;}
	return true;
}
...
function safeBeforcare(){
	if(!this.care.value){winjinAlert("請填寫登陸問候語","alert");return false;}
	return true;
}
...
function safeBefornickname(){
	if(!this.nickname.value){winjinAlert("請填寫暱稱","alert");return false;}
	return true;
}
...
function teamBeforeAddMember(){
	...
	if(!this.username.value){winjinAlert("沒有輸入使用者名稱","alert");return false;}
	if(!/^\w{4,16}$/.test(this.username.value)){winjinAlert("使用者名稱由4到16位的字母或數字組成","alert");return false;}
	if(!this.password.value){winjinAlert("請輸入密碼","alert");return false;}
	if(this.password.value.length<6){winjinAlert("密碼至少6位","alert");return false;}
	if(document.getElementById('cpasswd').value!=this.password.value){winjinAlert("兩次輸入密碼不一樣","alert");return false;}
	if(!this.fanDian.value){winjinAlert("請輸入返點","alert");return false;}
	if(parseFloat(this.fanDian.value)<0){winjinAlert("返點不能小於0%","alert"); return false;}
	if(parseFloat(this.fanDian.value)>parseFloat($(this.fanDian).attr('max'))){winjinAlert('返點不能大於'+$(this.fanDian).attr('max'),"alert"); return false;}
	var fanDianDiff= $(this.fanDian).attr('fanDianDiff');
	if((this.fanDian.value*1000) % (fanDianDiff*1000)){winjinAlert('返點只能是'+fanDianDiff+'%的倍數',"alert");return false;}
}
...
function userDataBeforeSubmitCode(){
	...
	if(!this.fanDian.value.match(/^[\d\.\%]{1,4}$/)) throw('請正確設定返點');
...
/**
 * 追号
 *  DAVID UPDATE 2016-03
 */
function setGameZhuiHao(data,num,bs){
	$.get('/index.php/index/zhuiHaoModal', function(html){
		$(html).dialog({
			title:'<input type="checkbox" checked name="zhuiHaoMode" value="1"/>中獎後停止追號</label>　追號期數：<span class="qs">0</span>　總金額：<span class="amount">0.00</span>USD',
			minWidth:600,
			height:400,
			modal:true,
			stack:false,
...
buttons:{
				"確定追號":function(){
					...
					$('tbody :checkbox:checked', this).each(function(){
						var $this=$(this),
						$tr=$this.closest('tr');
						data.push([$('td:eq(1)', $tr).text(), $('.money', $tr).val(), $('td:eq(4)', $tr).text()].join('|'));
					});
					
					if(!data.length){
						winjinAlert('追號至少選一期',"alert");
						return false;
					}
...
function gameMsgAutoTip(){
	...
	if(calcFun && (calcFun=window[calcFun]) && (typeof calcFun=='function')){
		try{
			...
			$('#game-tip-dom').text('預選共 '+obj.actionNum+' 註，金額 '+(gameGetMode()*gameGetmoney()*obj.actionNum).round(2)+' 元');
			$(".all-count").text(obj.actionNum);
			$(".all-amount").text((gameGetMode()*gameGetmoney()*obj.actionNum).round(2));
		}catch(err){
...
function gameActionRandom(number){
	...
	if($game.length<=1 || $game.attr('action')=='tz5xDwei'){
		winjinAlert('玩法暫不支援機選，請手動選擇號碼！');return;
	}		
...
function gameAddCode(code){
	...
	destroyWait();
	if(actionNo){
		winjinAlert('本期投注已截止，請下一期再投注',"alert");
		return false;
	}
	if($.isArray(code)){
	...
	try{
		...
		amount=code.mode * code.money * code.actionNum,
		$wei=$('#wei-shu'),
		...
		.append(
			$('<td>').data('value', code.actionNum).append('['+code.actionNum+'註]')
		)
		.append(
			$('<td>').data('value', amount).append(amount.round(2)+"元")
		)
		.append(
			$('<td>').append(code.money+'倍')
		)
		.append(
...
function gamePostCode(){
	...
	destroyWait();
	if(!actionNo){
		winjinAlert('取得投注期號出錯',"alert");
		return false;
	}
	var tipString='<span class="ui-wjicon-confirm"></span>確定要購買第<b>'+actionNo['actionNo']+'</b>期彩券？';
	tipString+='<br /><table width="100%"><tr><th>玩法</th><th>號碼</th><th>注数</th><th>倍数</th><th>模式</th></tr>';
	$('.touzhu-cont tr').each(function(){
		var $this=$(this);
...
function gameActionAddCode(){
	...
	if(userFanDian-fanDian> modeFanDian){
		...
		winjinAlert(modeName[mode]+'模式最大獎金只能為'+_amount.toFixed(2),"alert");
		return false;
	}
	
	// 单笔中奖限额
	var maxZjAmount=$slider.data().betZjAmount;
		//console.log('限额：%s, 中奖：%s', maxZjAmount, gameGetPl() * mode * ($('#money').val()||1));
	if(maxZjAmount){
		if($('#fandian-value').data('pl') * mode/2 * ($('#money').val()||1) > maxZjAmount){
			winjinAlert('單筆中獎獎金不能超過'+maxZjAmount+'元',"alert");
			return false;
		}
	}
	...
	if(calcFun && (calcFun=window[calcFun]) && (typeof calcFun=='function')){
		try{
			...
			if(maxBetCount && obj.actionNum>maxBetCount){
				winjinAlert('單筆投註註數最大不能超過'+maxBetCount+'註',"alert");
				return false;
			}
...
function confirmCancel(){
	var obj=$(this);
	var tipString='<span class="ui-wjicon-confirm"></span>是否確定撤單？';
		var wjDialog=$('#wanjinDialog').html(tipString).dialog({
		title:'溫馨提示',
		resizable: false,
		width:450,
...
function gameGetmoney(){
	...
	if(isNaN(txt=parseInt(txt))) throw('倍數設定不正確');
	return txt;
}
...
function tzAllSelect(){
	...
	if(this.has('.checked').length!=codeLen) throw('請選'+codeLen+'位元數');
...
function tzSscInput(){
	...
	if(str.length && str.length % codeLen == 0){
		if(/[^\d]/.test(str)) throw('投注有錯，不能有數字以外的字符。');
...
/*
* 六合彩玩法 ajax方法  DAVID UPDATE 2016
* 六合彩二中二
*/
function lhc_2z2(){
	var code=[], len=1,codeLen=parseInt(this.attr('length'));
