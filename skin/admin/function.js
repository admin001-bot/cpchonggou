...
function beforeAddMember(){
	if(!this.username.value.match(/[a-zA-z_]\w{4,15}/)) throw('使用者名稱由5到16位元字母或數字組成，前一位不能是數字');
	if(!this.password.value) throw('密碼不能為空');
	if(this.password.value.length<6 && this.password.value) throw('密碼不能少於6位');
	if(this.password.value != $('#cpasswd',this).val()) throw('確認密碼不一樣');
...
function userDataBeforeSubmitCode(){
	if(this.password.value.length<6&&this.password.value)throw('密碼不能少於6位');
	if(this.coinPassword.value.length<6&&this.coinPassword.value) throw('資金密碼不能少於6位');
	if(this.password.value||this.coinPassword.value){
		if(this.password.value==this.coinPassword.value) throw('密碼和資金密碼不能相同');
	}
	if(this.coin.value<0) throw('設定有問題');
	if(!this.fanDian.value.match(/^[\d\.\%]{1,4}$/)) throw('請正確設定返點');
...
// 弹出充值页
function rechargModal(uid){
	$.get('/admin778899.php/' + 'business/rechargeModal?uid='+uid, function(html){

		$(html).dialog({
			title:'使用者儲值',
			width:380,
			buttons:{
				"確定儲值":function(event, ui){
					var $this=$(this),
					...
					try{
						if(userRid==1){
							if(isNaN(uid)) throw('使用者ID不正確，使用者ID為使用者的數字ID');
							uid=parseInt(uid);
						};
						if(!amount.match(/^-?\d+(\.\d{0,2})?$/)) throw('儲值金額錯誤');
...
// 弹出彩金赠送页
function rechargModalget(uid){
	$.get('/admin778899.php/' + 'business/rechargeModalget?uid='+uid, function(html){
			...
			buttons:{
				"確定儲值":function(event, ui){
					...
					try{
						...
						if(!amount.match(/^-?\d+(\.\d{0,2})?$/)) throw('贈送金額錯誤');
						amount=parseFloat(amount).toFixed(2);
						//if(!isNaN(min) && min>amount) throw('充值金额不能少于'+min);
					}catch(err){
						error(err);
		...
		$(html).dialog({
			...
			buttons:{
				"添加":function(event, ui){
					...
					try{
						if(username=="")  throw('使用者名稱不能為空');
						if(!amount.match(/^-?\d+(\.\d{0,2})?$/)) throw('儲值金額錯誤');
						amount=parseFloat(amount).toFixed(2);
						if(liqType=="")  throw('類型不能為空');
						if(isNaN(liqType)) throw('類型必須是數字');
						if(info=="")  throw('info不能為空');
					}catch(err){
						error(err);
						return;
					}
...
function rechargeBeforeSubmit(){
	if(!this.rechargeAmount.value.match(/^\d+(\.\d+)?$/)) throw('請正確輸入金額');
}
...
function BeforeAddpay(){
	if(!this.name.value) throw('商家名稱不能為空');
	if(!this.name.value) throw('商家編號不能為空');
	if(!this.mkey.value) throw('商家密鑰不能為空');
}
...
function manageBeforeAddManager(){
	if(!this.username.value.match(/^[a-zA-Z_]\w{4,15}$/)) throw('使用者名稱不正確，使用者名稱由5到16位字母、數字或下畫線組成，頭位不能是數字');
	if(!this.password.value.match(/^.{6,32}$/)) throw('密碼由6到32個字元組成');
	if(this.password.value!=$('.cpwd', this).val()) throw('兩次輸入密碼不一樣');
}
