...
//			$scope.mdfBankData.address = My.getBank().subAddress;
			$scope.mdfBankData.cardNoTip = "舊卡"+My.getBank().cardNo;
//			$scope.mdfBankData.cardNo = My.getBank().cardNo;
		}
	};
})

.controller('LiveController', function($scope, $log, $state, My, Tools, UCenter, md5) {
	$log.debug("LiveController...");


	$scope.loadBalance=function(li){
		// layer.alert('aa');
		// console.log();
		 var name = My.getUserId();
		 // alert(li);
		 $scope.game = li;
		$('#'+li).html("<img class='loading' src='/images/loading.gif' border='0'>");
		 Tools.ajax({
			url: '/lives/balances',
			method:'POST',
			params:{game:li,username:name},
	...
	$scope.inmoney=function(li){
		var game = li;
		layer.prompt({title: '請輸入金額', formType: 4}, function(amount, index){
  	layer.close(index); 
...
.controller('OrderController', function($scope, $log, $state, My, Tools, UCenter, md5) {
