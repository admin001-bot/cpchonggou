...
.filter('lotteryResult', function($interpolate) {
	return function(nums, group) {
		...
		if(group == 'group1') {
			...
			results.push(((nums[0] + nums[1]) % 2 == 1) ? '單' : '雙');// 3
			results.push(nums[0] > nums[9] ? '龍' : '虎'); // 4
			results.push(nums[1] > nums[8] ? '龍' : '虎'); // 5
			results.push(nums[2] > nums[7] ? '龍' : '虎'); // 6
			results.push(nums[3] > nums[6] ? '龍' : '虎'); // 7
			results.push(nums[4] > nums[5] ? '龍' : '虎'); // 8
		}
		else if(group == 'group2') {
			results.push(total);// 1
			results.push((total >= 23) ? '大' : '小');// 2
			results.push((total % 2 == 1) ? '單' : '雙');// 3
			if(nums[0] > nums[4]) {
				results.push('龍'); // 4
			}
			...
			results.push(nums[0] > nums[4] ? '龍' : '虎');// 5
