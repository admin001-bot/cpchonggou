...
.controller('LotteryCtrl', function($scope, $timeout, $log, $filter, $stateParams, $ionicPopover, Lottery, Tools, My, $ionicScrollDelegate) {
	...
	var checkPlayMoney = function(play, betMoney, totalMoney) {
		...
		if (betMoney < minMoney) {
			Tools.tip('最小投注金額為：' + minMoney);
			return false;
		}
		
		if (betMoney > maxMoney) { 
			Tools.tip('最大投注金額為：' + maxMoney);
			return false;
		}
		
		if (totalMoney > maxTurnMoney) {
			Tools.tip('當前期設定的最大投注金額為：' + maxTurnMoney);
			return false;
		}
	...
	$scope.bet = function() {
		...
		if($scope.selectSub) {
			if($scope.selectSub.type == 'LXLW') {
				...
				confirmHtml += '<div>【合计】组数：<span class="t-blue" style="margin-right:15px">' + betCount + '</span>總金額：<span class="t-blue">' + totalMoney + '</span></div>';
			}
