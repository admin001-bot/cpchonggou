...
public function getGamenextNo($type, $time = null)
{
    ...
    if (!$return) {
        ...
        // 将 actionTime 设置为次日的时间
        if ($return) {
            $return['actionTime'] = date("Y-m-d H:i:s", strtotime($return['actionTime']) + 24 * 3600);
        }
    }
...
/* 	public function BJpk101(&$actionNo, &$actionTime, $time=null){
	...
	$actionNo = 1152*(strtotime(date('Y-m-d', $time))-strtotime('2018-10-10'))/3600/24+$actionNo+30817232;	
	}	
     //秒速时时彩
	...
	public function no0Hdx(&$actionNo, &$actionTime, $time = null){//极速分分彩
		$this->setTimeNo($actionTime, $time);
		$actionNo = date('Ymd', $time) . substr(10000 + $actionNo, 1);
	}
	...
	public function newahk3(&$actionNo, &$actionTime, $time = null){//湖北快3
		$this->setTimeNo($actionTime, $time);
		$actionNo = date('Ymd', $time) . substr(1000 + $actionNo, 1);
	}
	...
	public function newldpk10(&$actionNo, &$actionTime, $time=null){//伦敦PK10
		$this->setTimeNo($actionTime, $time);
		$actionNo = 480*(strtotime(date('Y-m-d', $time))-strtotime('2017-05-11'))/3600/24+$actionNo+778899;
	}	
	...
	public function noHdCQSSC(&$actionNo, &$actionTime, $time=null){
		...
		if($actionNo==0||$actionNo==59){
			$actionNo=date('Ymd59', $time - 24*3600);
	...
	public function noxHd1(&$actionNo, &$actionTime, $time = null) {
    $this->setTimeNo($actionTime, $time);
	...
	*/
	...
	public function BJpk10(&$actionNo, &$actionTime, $time=null){
		$this->setTimeNo($actionTime, $time);
		$actionNo = 44*(strtotime(date('Y-m-d', $time))-strtotime('2007-11-18'))/3600/24+$actionNo+548859-4928;
	}
	public function Kuai8(&$actionNo, &$actionTime, $time=null){
		$this->setTimeNo($actionTime, $time);
		$actionNo = 179*(strtotime(date('Y-m-d', $time))-strtotime('2004-09-19'))/3600/24+$actionNo-1292*2-3230;
