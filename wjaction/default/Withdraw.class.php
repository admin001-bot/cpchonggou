...
class Withdraw extends WebLoginBase{
	...
	public final function submitdo(){
		...
		if(!ctype_digit($para['amount'])){echo '提現金額包含非法字符';exit;} //throw new Exception('提现金额包含非法字符');
		if($para['amount']<=0){echo '提現金額只能為正整數';exit;} //throw new Exception("提现金额只能为正整数");
// 		if($para['amount']>$this->user['coin']){echo '提款金額大於可用餘額，無法提款';exit;} 
		//throw new Exception("提款金额大于可用余额，无法提款");
		if($this->user['coin']<=0){echo '可用餘額為零，無法提款';exit;} //throw new Exception("可用余额为零，无法提款");
...
// 		if($this->time < $fromTime || $this->time > $toTime ){echo "提現時間：從".$this->settings['cashFromTime']."到".$this->settings['cashToTime'];exit;} 
		...
		if($this->settings['cashMinAmount']){
			...
			if($rechargeAmount){
				...
				if(floatval($cashAmout)<floatval($rechargeAmount)){echo "消費滿".$this->settings['cashMinAmount']."%才能提現";exit;}  
		...
		try{
			$this->freshSession();
			if($this->user['coinPassword']!=md5($para['coinpwd'])){echo '提款密碼不正確';exit;} //throw new Exception('提款密码不正确');
			unset($para['coinpwd']);
			
			if($this->user['coin']<$para['amount']){echo '你帳號資金不足';exit;} //throw new Exception('你帐户资金不足');
			...
			if(!$this->insertRow($this->prename .'member_cash', $para)){echo '提交提現請求出錯';exit;} //throw new Exception('提交提现请求出错');
			...
			$this->rollBack();
			  error_log('提现错误：'.$e->getMessage(), 3, $_SERVER['DOCUMENT_ROOT']."/saveerror.log");
			//return 9999;
			throw $e;
		}
	}
