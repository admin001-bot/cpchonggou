	...
	public final function getTKTip(){
		...
		if($data=$this->getCol($sql)){
			...
			if($state==4){
				return array(
					'flag'=>true,
					'message'=>'提款失敗！<br/>原因：'.$info
				);
			}else{
				return array(
					'flag'=>true,
					'message'=>'提款成功！<br/>金額：'.$amount.'USD'
				);	
			}///end state
		}
	}
	...
	public final function getCZTip(){
		...
		if($data=$this->getCol($sql)){
			...
			if($CZAmount>0){
				return array(
					'flag'=>true,
					'message'=>'儲值成功！<br>系統儲值：'.$CZAmount.'USD'
				);
			}else{
				return array(
					'flag'=>true,
					'message'=>'扣款成功！<br>系統扣款：'.abs($CZAmount).'USD'
				);
		   }/////
		}
	...
	public final function getYKTip($type, $ctionNo){
	 ...
	 if($type && $ctionNo){
		...
		if($this->getCol("select id from {$this->prename}bets ".$whereStr)){
			$ykMoney=$this->getValue("select IFNULL(sum(bonus-(money*(1-fanDian/100))),'0') tMoney from {$this->prename}bets ".$whereStr."");
			
			if($ykMoney>0){
				$messager=$czName." 第".$ctionNo."期：<br />盈虧 <font style='color:#F00;font-weight:bold;font-size:14px;'>".round($ykMoney,2)."</font> USD";
			
			}else{
				$messager=$czName." 第".$ctionNo."期：<br />盈虧 <font style='color:#060;font-weight:bold;font-size:14px;'>".round($ykMoney,2)."</font> USD";
