...
$checklogin=array();
//查询系统配置
$checklogin['isweihu']=0;
$days=$this->settings['days'];
$times=$this->settings['times'];
//分割
$arrdays=explode(',',$days);
$arrtimes=explode('-',$times);
//获取当前时间
if(in_array(date('w'),$arrdays)){
	$nowtime=(float)date('H.i');
	if($nowtime>(float)str_replace(':','.',$arrtimes[0])&&$nowtime<(float)str_replace(':','.',$arrtimes[1])){
		$checklogin['isweihu']=1;
	}
}
...
$checklogin['ison']=$this->user['ison'];
