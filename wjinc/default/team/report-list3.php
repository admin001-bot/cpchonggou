<?php
	...
	if($para['parentId']=intval($para['parentId'])){
		...
		$userWhere=" and u.zparentId={$para['parentId']} and u.type=1  ";
		}elseif($para['type']==3 && $para['parentId']==$this->user['uid']){
		$userWhere=" and u.gudongId={$para['parentId']} and u.type=2  ";
		}else{
...
<table width="100%" class='table_b'>
	...
	<?php } ?>
        ...
        <?php if(intval($para['userType']) !=1){ ?>
		<tr>
			<td><span class="spn9"><?php if(intval($para['userType'])==2){echo '直接下級';}else if(intval($para['userType'])==3){echo '所有下級';}else{echo '總結';}?></span></td>
