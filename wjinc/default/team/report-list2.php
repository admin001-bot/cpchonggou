<?php
	...
	if($para['parentId']=intval($para['parentId'])){
		...
		$uid=$uParentId;
	}elseif($para['username'] && $para['username']!='使用者名稱'){
		// 用户名限制
		$para['username']=wjStrFilter($para['username']);
		if(!ctype_alnum($para['username'])) throw new Exception('使用者名稱包含非法字元,請重新輸入');
		$uid=$this->getValue("select uid from {$this->prename}members where username=? and (parentId={$this->user['uid']} or zparentId={$this->user['uid']} or gudongId={$this->user['uid']} )",$para['username']);
		$userWhere="and u.username='{$para['username']}' and (parentId={$this->user['uid']} or zparentId={$this->user['uid']} or gudongId={$this->user['uid']} )";
	}else{
		$userWhere=" and (parentId={$this->user['uid']} or zparentId={$this->user['uid']} or gudongId={$this->user['uid']} )";  
	}
	$userWhere3="and (parentId={$this->user['uid']} or zparentId={$this->user['uid']} or gudongId={$this->user['uid']} )";
...
<table width="100%" class='table_b'>
	...
	?>
		<tr>
			...
			<td><?=$this->ifs($var['rebateMoneyDouble'], '--')?></td>
            <td><?=$this->ifs($var['zjAmount']-$var['betAmount']+$var['rebateMoneyDouble'], '--')?></td>
            <td>
                <?php if(!$noChildren){?>
	...
	<?php } ?>
        ...
        <?php if($para['userType']==1 || ($para['userType']==0 && !$para['parentId']) ||($para['username'] && $para['username']!='使用者名稱')){//------------------------------------------
		...
		<tr>
			...
			<td><?=$this->ifs($count['rebateMoneyDouble'], '--')?></td>
            <td><?=$this->ifs($count['zjAmount']-$count['betAmount']+$count['rebateMoneyDouble'], '--')?></td>
			<td></td>
		</tr>
		<tr>
			...
			<td><?=$this->ifs($all['rebateMoneyDouble'], '--')?></td>
			<td><?=$this->ifs($all['zjAmount']-$all['betAmount']+$all['rebateMoneyDouble'], '--')?></td>
			<td></td>
		</tr>
		<?php }else{//----------------------------------------
...
<?php 
	$this->display('inc_page.php',0,$list['total'],$this->pageSize, "$rel-{page}");
?>
</div>