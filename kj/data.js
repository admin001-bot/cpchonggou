		...
		res.on("end", function(){
			try{
				try{
					...
					if(data.type=='73' || data.type=='81' || data.type=='100' || data.type=='101' || data.type=='102' || data.type=='103' || data.type=='104' || data.type=='105' || data.type=='106' || data.type=='107' || data.type=='109' || data.type=='110' || data.type=='111' || data.type=='112' || data.type=='113' || data.type=='122' || data.type=='51' || data.type=='72' || data.type=='52'){
					liRunData(data, conf);
					}else{
	...
	client.query("insert into ssc_data(type, time, number, data) values(?,?,?,?)", [data.type, data.time, data.number, data.data], function(err, result){
		if(err){
			...
			if(err.number==1062){
				...
				restartTask(conf, 10000/1000, true);
