$(function(){
	...
	$('#textarea-code').live('change', function(){
		var str=$(this).val();
		if(/[a-zA-Z]+/.test(str)){
			winjinAlert('投注號碼不能包含字母字符',"alert");
			$(this).val('');
		}
	});
	...
	.click(function(){
		...
		len=bet.length
		if(len==0){
			winjinAlert('請選投注',"alert");
			return false;
		}else if(len>1){
			winjinAlert('只能針對一個方案發動追號！',"alert");
			return false;
		}
	...
	$('.zhui-hao-modal tbody .money').live('change', function(e){
		var $this=$(this);
		var re=/^[1-9][0-9]*$/;
		if(!re.test($this.val())){winjinAlert('倍數只能為正整數',"alert");$this.val(1);return;}
	...
	$('.dmtm :input.code').live('click',function(event){
		...
		if($('.code.checked[value=' + this.value +']', $dom).not(this).length==1){
			$this.removeClass('checked');
			winjinAlert('選擇膽碼不能與拖曳碼相同',"alert");
			return false;
		}
	});
	...
	if(getVoiceStatus()=='off'){
		$('#voice').removeClass('voice-on').addClass('voice-off').attr('title', '聲音關閉，點選開啟');
	}
});
var MaxZjCount={
	ds:function(){
...
function voiceKJ(){
	...
	if(getVoiceStatus()!='off'){
		...
		$dom.attr('class','voice-off').attr('title', '聲音關閉，點選開啟');
	}else{
		setVoiceStatus(true);
		$dom.attr('class','voice-on').attr('title', '聲音開啟，點選關閉');
	}
}
// 加载玩法消息
